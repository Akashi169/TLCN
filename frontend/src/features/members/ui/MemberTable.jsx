...
function TierBadge({ rankName, tier }) {
  if (tier === 'diamond' || (rankName && rankName.includes('Kim Cương'))) {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gradient-to-r from-indigo-50 via-sky-50 to-cyan-50 text-indigo-950 border border-cyan-300/90 shadow-2xs text-xs font-extrabold tracking-wide">
  ...
  if (tier === 'gold' || (rankName && rankName.includes('Vàng'))) {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gradient-to-r from-amber-50 to-yellow-100 text-amber-950 border border-amber-300/90 shadow-2xs text-xs font-bold tracking-wide">
  ...
  if (tier === 'silver' || (rankName && rankName.includes('Bạc'))) {
    return (
      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 text-slate-800 border border-slate-300 text-xs font-bold">
...
function AccountStatusBadge({ status }) {
  if (status === 'ACTIVE' || status === 'active' || status === 'online') {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200/80 rounded-full font-mono text-[11px] font-bold">
  ...
  if (status === 'LOCKED' || status === 'locked') {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-50 text-rose-800 border border-rose-200/80 rounded-full font-mono text-[11px] font-bold">
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 text-amber-950 border border-amber-200/80 rounded-full font-mono text-[11px] font-bold">
...
/**
 * MemberTable component matching exact prompt specifications:
 * Table Columns: UID | Hội viên | Hạng | Số dư | Điểm | Trạm & trạng thái hoạt động | Trạng thái tài khoản | Lần đăng nhập cuối | Thao tác
 */
        ...
        <table className="w-full text-left border-collapse min-w-[1150px]">
          <thead>
            <tr className="bg-slate-100/90 text-slate-800 text-[11px] uppercase tracking-wider border-b border-slate-200 font-extrabold select-none">
              ...
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[100px]">UID</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[200px]">Hội Viên</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[140px]">Hạng</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 text-right pr-6 min-w-[160px]">Số Dư</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 text-right pr-6 min-w-[120px]">Điểm</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[200px]">Trạm &amp; Trạng Thái Hoạt Động</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[160px]">Trạng Thái Tài Khoản</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 min-w-[170px]">Lần Đăng Nhập Cuối</th>
              <th className="py-3.5 px-4 font-extrabold text-slate-800 text-right pr-6 min-w-[160px]">Thao Tác</th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 font-mono font-bold text-sky-800 whitespace-nowrap">
                    <span className="px-2 py-0.5 bg-slate-100 border border-slate-200/80 rounded text-[11px]">
                  ...
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-xs text-slate-800 shrink-0 shadow-2xs">
                  ...
                  <td className="py-3.5 px-4 text-right pr-6 whitespace-nowrap">
                    ...
                    <span className="text-[10px] text-slate-600 font-semibold ml-1">pts</span>
                  </td>
